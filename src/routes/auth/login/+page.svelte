<script lang="ts">
	...
	function friendlyAuthError(message: string) {
		...
		if (normalized.includes('email rate limit exceeded') || normalized.includes('rate limit exceeded')) {
			return 'Too many auth attempts happened just now. Please wait a little, then try again.';
		}
		return message;
	}
	...
	async function resendConfirmation() {
		...
		resendMessage = `We sent a fresh confirmation link to ${email}.`;
		resendLoading = false;
	}
</script>
...
<div class="auth-page">
	...
	<nav class="auth-nav">
		...
		<a href="/auth/signup{joinCode ? `?join=${joinCode}` : ''}" class="auth-nav-link">New here? Create account →</a>
	</nav>
			...
			<form class="auth-form" onsubmit={handleLogin}>
				...
				{#if needsConfirmation}
					<div class="confirm-panel">
						<p class="confirm-copy">Confirm your email first, then return here and sign in.</p>
						<button
							type="button"
							class="resend-link"
